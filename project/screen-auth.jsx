...
function SplashScreen({ onContinue }) {
  ...
  return (
    <div className="screen" style={{ alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'absolute', bottom: 60, color: 'var(--text-3)', fontSize: 12, letterSpacing: 1 }}>
    ...
    <div className="screen">
      ...
      <div style={{ position: 'relative', flex: 1, padding: '88px 28px 40px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ marginTop: 72 }}>
          <div style={{ fontSize: 38, fontWeight: 700, lineHeight: 1.12, letterSpacing: -0.5 }}>
            欢迎回来,<br/>
            <span style={{ color: 'var(--lime)' }}>开始今天</span>的训练吧
          </div>
          <div style={{ marginTop: 16, color: 'var(--text-2)', fontSize: 15, lineHeight: 1.5 }}>
            AI 教练全程陪练 · 智能识别器械<br/>
            为你量身打造科学训练计划
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          >
            {loading ? (
              <span style={{ display: 'inline-block', width: 18, height: 18, border: '2px solid rgba(255,255,255,.4)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
            ) : <Icon.WeChat size={22}/>}
            微信一键登录
          </button>
        ...
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'flex-start', gap: 8 }}>
          <button
            ...
            style={{
              ...
              border: agreed ? '0' : '1.5px solid var(--text-3)',
              background: agreed ? 'var(--lime)' : 'transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <div style={{ fontSize: 12, color: 'var(--text-3)', lineHeight: 1.5 }}>
            阅读并同意 <span style={{ color: 'var(--lime)' }}>《用户协议》</span>
            和 <span style={{ color: 'var(--lime)' }}>《隐私政策》</span>，
            未注册手机号将自动创建账号
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            <button key={g.id} onClick={() => setGoal(g.id)} style={{
              padding: 18, borderRadius: 18, textAlign: 'left',
              background: goal === g.id ? 'rgba(206,255,77,0.12)' : 'var(--surface)',
              border: '1px solid ' + (goal === g.id ? 'var(--lime)' : 'var(--line)'),
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            <button key={g.id} onClick={() => setLevel(g.id)} style={{
              ...
              border: '1px solid ' + (level === g.id ? 'var(--lime)' : 'var(--line)'),
            }}>
              <div style={{ fontSize: 16, fontWeight: 600 }}>{g.label}</div>
              <div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 3 }}>{g.desc}</div>
            </button>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16 }}>
            {[2,3,4,5,6].map(d => (
              <button key={d} onClick={() => setDays(d)} style={{
                ...
                background: days === d ? 'var(--lime)' : 'var(--surface)',
                color: days === d ? '#0a0d0a' : '#fff',
                fontWeight: 600, fontSize: 16,
                border: '1px solid ' + (days === d ? 'var(--lime)' : 'var(--line)'),
              }}>{d}</button>
            ))}
          </div>
          <div style={{ marginTop: 36, padding: 16, background: 'var(--surface)', borderRadius: 16, fontSize: 13.5, color: 'var(--text-2)', lineHeight: 1.6 }}>
  ...
  return (
    <div className="screen">
      <div style={{ padding: '60px 24px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => step > 0 ? setStep(step-1) : null} className="iconbtn" style={{ visibility: step > 0 ? 'visible' : 'hidden' }}>
      ...
      <div style={{ padding: '12px 24px 36px' }}>
        <LimeButton full onClick={next} disabled={!current.valid}>
